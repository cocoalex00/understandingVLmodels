...
class FeatureExtractor:
    ...
    def _save_feature(self, file_name, feature, info):
		...
		#### ALEJANDRO ######
		# Add different file name to match label csv file
        delimiter = "-"
        toremove = ["","home", "mount", "Places365", "train_256", "val_256", "test_256", "images", "prueba"]
        newname = [element for element in file_name.split("/") if element not in toremove]
        newname = delimiter.join(newname).split(".")[0]

        np.save(os.path.join(self.args.output_folder, newname), info)
    ...
    def extract_features(self):
        ...
        if os.path.isfile(image_dir):
            ...
            self._save_feature(image_dir, features[0], infos[0])
						...
						############## CODE ADDED BY ALEJANDRO HERNANDEZ (recursive search in subfolders)################################
