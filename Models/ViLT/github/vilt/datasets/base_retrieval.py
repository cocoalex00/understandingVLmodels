...
class PlacesDatasetBaseRetrieval(torch.utils.data.Dataset):
    def __init__(
        self,
        data_dir: str,
        labels_path: str,
    ...
    def get_raw_image(self, index, image_key="img"):
    ...
    def get_text(self, raw_index):
        ...
        if self.val:
            ...
            category = self.all_texts[index]

            text = self.labels_to_text[category]    
            label = self.table["label"][index].as_py()[0]
        else:
            if random.choice([0,1]) == 1:
                category = self.table["label"][index].as_py()[0]
                ...
                randomID = random.choice(list(set(range(0, 364)) - set([int(category)])))
